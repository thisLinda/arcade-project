...
This file provides the game loop functionality (update entities and render), draws the initial game board on the screen, and then calls the update and render methods on your player and enemy objects (defined in your app.js).
 ...
 A game engine works by drawing the entire game screen over and over, kind of like a flipbook you may have created as a kid. When your player moves across the screen, it may look like just that image/character is moving or being drawn but that is not the case. What's really happening is the entire "scene" is being drawn over and over, presenting the illusion of animation. This engine makes the canvas' context (ctx) object globally available to make writing app.js a little simpler to work with.
 */

const Engine = (function (global) {
  /* Predefine the variables we'll be using within this scope,create the canvas element, grab the 2D context for that canvas set the canvas elements height/width and add it to the DOM.
   */
   ...
   */
  function main() {
    /* Get our time delta information which is required if your game requires smooth animation. Because everyone's computer processes instructions at different speeds we need a constant value that would be the same for everyone (regardless of how fast their computer is) - hurray time!
     */
    let now = Date.now();
  ...
  function render() {
    ...
    for (row = 0; row < numRows; row++) {
      for (col = 0; col < numCols; col++) {
        /* The drawImage function of the canvas' context element requires 3 parameters: the image to draw, the x coordinate to start drawing and the y coordinate to start drawing. We're using our Resources helpers to refer to our images so that we get the benefits of caching these images, since we're using them over and over.
         */
        ctx.drawImage(Resources.get(rowImages[row]), col * 101, row * 83); //101 and 83 are offset to prevent overlap, will need to be changed for responsive utilizing media queries, see Rodrick's video at 18:54
      }
    }
  ...
  /* This function is called by the render function and is called on each game tick. Its purpose is to then call the render functions you have defined on your enemy and player entities within app.js
   */
